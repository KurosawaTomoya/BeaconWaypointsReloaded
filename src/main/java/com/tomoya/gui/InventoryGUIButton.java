...
import java.util.function.Consumer; // Hàng real của Java chuẩn meta đây
...
public class InventoryGUIButton {
    ...
    public void setPlayerHead(String username) {
        ...
        try {
            // Đã thanh tẩy Reflection, gọi API native của Paper 1.21+ bao mượt
    ...
    public void onClick(InventoryClickEvent e) {
        if (this.onClick != null) {
            try {
                ...
                e.setCancelled(true);
                BeaconWaypointsReloaded.getPlugin().getLogger().severe(ex.toString());
                StringBuilder stackTraceStr = new StringBuilder(ex.getMessage() != null ? ex.getMessage() : "Error").append("\n");
                for (StackTraceElement frame : ex.getStackTrace()) {
                    stackTraceStr.append(frame.toString()).append("\n");
                }
                BeaconWaypointsReloaded.getPlugin().getLogger().severe(stackTraceStr.toString());
            }
        }
    }
    ...
    public boolean equals(InventoryGUIButton compare) {
        return this.name.equals(compare.getName()) && this.description.equals(compare.getDescription()) && this.material == compare.getMaterial();
    }
}