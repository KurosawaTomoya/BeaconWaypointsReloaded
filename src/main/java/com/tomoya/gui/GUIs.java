...
public class GUIs {
    ...
    // TRICK LỎ: Danh sách lưu tạm mấy đứa vừa bấm "Mở menu mặc định"
    public static final Set<UUID> bypassVanillaBeacon = new HashSet<>();
    ...
    public static void waypointIconPickerMenu(Player player, Waypoint waypoint, InventoryGUI previousGUI) {
        ...
        for (String iconStr : config.getStringList("waypoint-icons")) {
            try {
                ...
                BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("waypoint-icon-not-found") + " " + iconStr);
            }
        }
    ...
    public static void beaconMenu(Player player, Waypoint waypoint) {
        ...
        vanillaBeaconButton.setOnClick(e -> {
            // Trick mở vanilla beacon không dùng NMS
            player.closeInventory();
            bypassVanillaBeacon.add(player.getUniqueId());
            player.sendMessage(ChatColor.YELLOW + "Click the beacon again to open the vanilla menu!");
        });
    ...
    public static void managePrivateWaypointAccessMenu(Player player, Waypoint waypoint, InventoryGUI previousGUI) {
        ...
        for (UUID uuid : waypoint.getSharedPlayers()) {
            String playerName = BeaconWaypointsReloaded.getWaypointManager().getPlayerUsername(uuid);
    ...
    public static void confirmRemovePlayerAccessMenu(Player player, UUID sharedPlayerUUID, Waypoint waypoint, InventoryGUI previousGUI) {
        ...
        String sharedPlayerName = BeaconWaypointsReloaded.getWaypointManager().getPlayerUsername(sharedPlayerUUID);
