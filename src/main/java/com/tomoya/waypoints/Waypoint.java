...
public class Waypoint implements Cloneable {
    ...
    public Waypoint(JSONObject jsonWaypoint) {
        ...
        if (jsonName == null) {
            dataError = true;
            BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("missing-attribute") + " \"name\"");
        }
        ...
        if (jsonX == null) {
            dataError = true;
            BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("missing-attribute") + " \"x'\"");
        }
        ...
        if (jsonY == null) {
            dataError = true;
            BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("missing-attribute") + " \"y\"");
        }
        ...
        if (jsonZ == null) {
            dataError = true;
            BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("missing-attribute") + " \"z\"");
        }
        ...
        if (jsonWorldName == null) {
            dataError = true;
            BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("missing-attribute") + " \"world\"");
        }
        else {
        ...
        if (jsonIcon == null) {
            dataError = true;
            BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("missing-attribute") + " \"icon\"");
        }
        ...
        if (jsonOwnerUUID == null) {
            dataError = true;
            BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("missing-attribute") + " \"ownerUUID\"");
        }
        ...
        if (jsonIsWaypoint == null) {
            dataError = true;
            BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("missing-attribute") + " \"isWaypoint\"");
        }
        else this.isWaypoint = Boolean.parseBoolean(jsonIsWaypoint.toString());

        if (dataError) {
            BeaconWaypointsReloaded.getPlugin().getLogger().severe(languageManager.getString("missing-attributes"));
        }
        else {
            Object jsonPinned = jsonWaypoint.get("pinned");
            if (jsonPinned == null) {
                BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("pinned-default"));
                this.pinned = false;
            }
            ...
            if (jsonPlayersDiscovered == null) {
                BeaconWaypointsReloaded.getPlugin().getLogger().warning(languageManager.getString("players-discovered-default"));
            }
            ...
            if (jsonPlayersDiscoveredArr != null) {
                for (Object jsonPlayer : jsonPlayersDiscoveredArr) {
                    try {
                        ...
                        BeaconWaypointsReloaded.getPlugin().getLogger().severe(languageManager.getString("error-reading-players-discovered"));
                    }
                }
            }
            ...
            if (jsonSharedPlayers != null) {
                for (Object jsonPlayer : jsonSharedPlayers) {
                    try {
                        ...
                        BeaconWaypointsReloaded.getPlugin().getLogger().severe(languageManager.getString("error-reading-shared-player"));
                    }
                }
            }
        }
        ...
        if (isActive != 0) {
            ...
            for (int blockX = beaconLoc.getBlockX() - 1; blockX <= beaconLoc.getBlockX() + 1; blockX++) {
                for (int blockZ = beaconLoc.getBlockZ() - 1; blockZ <= beaconLoc.getBlockZ() + 1; blockZ++) {
                    Material baseBlockType = Objects.requireNonNull(beaconLoc.getWorld()).getBlockAt(blockX, beaconLoc.getBlockY() - 1, blockZ).getType();
                    // Thay thế VersionWrapper cũ rích bằng Tag API siêu mượt
                    if (!Tag.BEACON_BASE_BLOCKS.isTagged(baseBlockType)) {
                        isActive = 0;
                        break;
                    }
                }
